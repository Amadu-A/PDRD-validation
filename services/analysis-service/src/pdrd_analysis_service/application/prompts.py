...
1. Сравнивается ли один и тот же объект,
   элемент, строка или связь; либо разные
   элементы, для которых сам документ явно
   задаёт обязательную связь.
...
7. Не используется ли physical PDF page/page_number
   как будто это значение проектного поля
   "Лист" или "Листов".

8. Если сравнивается количество,
   доказана ли самим листом либо N/T/U
   конкретная связь количества между объектами.

Проверяй условия, применимые к типу замечания.
Если обязательное для него условие не выполняется,
НЕ создавай source-less finding. Точное повторение
одной позиционной подписи у разных элементов
является конкретным фактом для needs_review;
оно не доказывает само по себе нарушение нормы.
...
- отличие физического индекса PDF-страницы
  от проектного значения "Лист";

- разное количество разных типов оборудования,
  если документ или N/T/U не задаёт
  обязательную связь между этими количествами;

- разные IP, напряжение, диаметр, мощность,
  температура или другие характеристики
  РАЗНЫХ объектов, если документ не показывает,
  что сравнивается одно и то же свойство
...
- физический индекс PDF-страницы,
  который backend передаёт как page_number,
  является transport metadata,
  а НЕ проектным реквизитом "Лист";

- не сравнивай page_number
  с видимым полем "Лист" как два значения
  одного проектного свойства;

- два видимых проектных номера
  можно сравнивать только если сам документ
  явно показывает, что это одна система нумерации;

- разность количества двух классов оборудования
  не доказывает ошибку;

- перед сравнением количеств установи
  явную one-to-one, one-to-many,
  комплектную или иную обязательную связь;

- замечание о внутренней несогласованности
  формируй, когда действительно сравниваются
  один объект, одна характеристика, одно
  обозначение или один и тот же scope,
  либо документ прямо задаёт связь между
...
- similarity/retrieval score используется
  только для поиска кандидатов;
- высокий retrieval score НЕ является
  доказательством применимости N-source;
...
- physical_pdf_page/page_number является
  служебной metadata и не доказывает
  значение проектного поля "Лист";
- различие количества разных объектов
  само по себе не доказывает,
  что между ними нарушена требуемая кратность;
...
Физический индекс PDF-страницы
(transport metadata, НЕ проектное поле "Лист"):
{page_number}
...
применимые SOURCE-LESS проверки из глобальных
правил: сопоставимый объект или документом
заданная связь объектов, сопоставимое свойство,
общий смысловой scope и конкретное evidence.

physical PDF page index является
только transport metadata.
Не используй его как значение поля "Лист".

Различие количества разных объектов
допустимо считать проблемой только тогда,
когда документ либо N/T/U прямо задаёт
обязательную связь между этими количествами.
        ...
        findings_payload.append(
            {
                "finding": {
                    ...
                    "physical_pdf_page": finding.page,
...
ВАЖНАЯ СЕМАНТИКА METADATA.

Поле physical_pdf_page в DATA:
- является transport metadata backend;
- используется для маршрутизации результата;
- НЕ является автоматически проектным полем "Лист";
- НЕ является evidence инженерного противоречия.

Нельзя считать различие между physical_pdf_page
и видимым значением "Лист" ошибкой.

Два видимых проектных значения номера листа
можно сравнивать только если evidence показывает,
что они относятся к одной и той же
проектной системе нумерации.
...
8. candidate сравнивает physical_pdf_page
   с проектным полем "Лист" или "Листов"
   как будто это одна система нумерации,
   хотя DATA этого не доказывает;

9. candidate объявляет ошибкой разное количество
   РАЗНЫХ типов оборудования,
   хотя evidence/N/T/U не задаёт
   обязательную связь или кратность
   между этими количествами;

10. candidate сравнивает IP, напряжение,
    диаметр, мощность, давление, температуру
    или другую характеристику РАЗНЫХ объектов
    как одну характеристику одного объекта.

Примеры unsupported quantity assumptions:
- 4 виброкомпенсатора против 3 насосов;
- 12 кранов против 3 насосов;
- 4 единицы арматуры одного типа против 5 котлов.

Такие числа сами по себе НЕ доказывают ошибку.

Если evidence прямо задаёт one-to-one,
комплектность, требуемую кратность
или два значения количества ОДНОГО tagged object,
это уже другая ситуация и candidate может быть keep.
...
- разные значения количества ОДНОГО
  идентифицированного объекта/позиции
  в двух сопоставимых частях документа;

- нарушение явно заданной документом
  количественной связи или кратности;
...
- НО отсутствие доказанной связи
  между двумя сравниваемыми объектами
  не является основанием keep;

- если сама связь X↔Y придумана моделью,
  candidate является unsupported assumption;
...
Retrieval score и место candidate в выдаче
используются только для recall.

Даже высокий score НЕ доказывает
семантическую применимость нормы.
...
Если норматив описывает похожую дисциплину,
но другое требование, объект,
условие применения или смысловой scope,
не выбирай его.
