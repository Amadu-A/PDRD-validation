...
- bbox = минимальная хорошо различимая область, непосредственно связанная
  с finding.

Правила:
- верни ровно один location для каждого finding_id;
- не меняй finding_id;
- status=located используй, когда на изображении можно указать конкретную
  видимую область, подтверждающую finding;
- если finding говорит об отсутствующем элементе, непрочерченной связи,
  пропущенной арматуре, маркировке или другом визуально отсутствующем объекте,
  локализуй ОБЛАСТЬ ДОКАЗАТЕЛЬСТВА ОТСУТСТВИЯ: участок линии, соседние элементы,
  концы разрыва, строку таблицы или иной контекст, где этот элемент должен
  проверяться;
- для отсутствующего объекта НЕ придумывай bbox самого отсутствующего объекта:
  bbox должен охватывать только реально видимый контекст проверки;
- status=unlocated, bbox=null используй только если нельзя надёжно определить
  ни сам объект, ни область доказательства по изображению, PAGE TEXT и FINDING;
- если finding содержит обозначения вроде T1.1, Т1.1, XT1, ЩР-1, используй их
  как визуальные ориентиры; латинские и кириллические похожие символы могут
  обозначать один и тот же инженерный тег;
- не придумывай bbox ради заполнения schema;
- если finding относится к тексту или таблице, обведи конкретную строку,
  ячейку или блок;
- если finding относится к существующему объекту или элементу схемы, обведи
  этот объект;
- если доказательство сравнивает две удалённые области, выбери область,
  где наиболее явно виден ошибочный факт;
- если на схеме виден разрыв соединения, обведи оба ближайших конца и сам
  промежуток одной минимальной областью;
